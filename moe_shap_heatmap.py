...
def get_shap_explainer(model, X, n_background=10):
    ...
    if hasattr(model, "named_steps") and "scaler" in model.named_steps:
        ...
        if hasattr(scaler, "feature_names_in_"):
            ...
            if missing_cols:
                print(f"⚠️ Detected {len(missing_cols)} missing columns — aligning test features with training-time columns")
    ...
    if isinstance(model_store, dict):
        ...
        for s in solute_order:
            ...
            if m is None:
                ...
                pairs.append((m, s))
        if missing:
            print(f"ℹ️ {len(missing)} solutes missing in model_store (skipping): {missing[:5]}...")
    else:
